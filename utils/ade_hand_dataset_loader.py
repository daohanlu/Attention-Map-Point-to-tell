...
class AdeHandDatasetLoader():
    ...
    def ade_label_to_voc_label(self, image):
        # image[image == [13, 13, 13]] = [15, 15, 15]
        # image[image == [16, 16, 16]] = [11, 11, 11]
        # image[image == [20, 20, 20]] = [9, 9, 9]

        mask = (image != 13) & (image != 16) & (image != 20) & (image != 200)
        image[mask] = 0 # get rid of unwanted masks
        image[image == 13] = 15
        image[image == 16] = 11
        image[image == 20] = 9
        image[image == 200] = 1 # replace finger with airplane
        return image
